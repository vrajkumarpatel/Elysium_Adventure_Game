...
def choose(options):
    """Render numbered options and return the chosen index as a string.

    Args:
        options (list[str]): list of option descriptions

    Returns:
        str: chosen number as a string, e.g., "1"
    """
    print("")
    for i, opt in enumerate(options, start=1):
        idx = colorize(f"[{i}]", fg="cyan", bold=True)
        print(f"  {idx} {opt}")
    inp = input(colorize(f"Select 1-{len(options)}: ", fg="yellow", bold=True)).strip()
    valid = {str(i) for i in range(1, len(options) + 1)}
    while inp not in valid:
        inp = input(colorize(f"Please enter a number between 1 and {len(options)}: ", fg="yellow")).strip()
    return inp
...
def print_art(filename: str, fg: str = "cyan") -> None:
    ...
    try:
        with open(path, "r", encoding="utf-8") as f:
            art = f.read()
        print(colorize(art, fg=fg, bold=True))
    except Exception:
        pass
